...
describe('TaskTracker', () => {
  ...
  describe('list', () => {
    it('should return a list of tasks', async () => {
      // Arrange
      vi.mocked(mockFilesystem.readFile).mockResolvedValue(JSON.stringify(mockTasks));
      ...
      if (E.isRight(result)) {
        expect(result.right).toEqual(mockTasks.map(Task.decode).filter(E.isRight).map(r => r.right));
      }
      expect(mockFilesystem.readFile).toHaveBeenCalledWith('tasks.json', 'utf-8');
    });
  ...
  describe('add', () => {
    it('should add a new task and return the updated list', async () => {
      ...
      vi.mocked(mockFilesystem.readFile)
        ...
        .mockResolvedValueOnce(JSON.stringify([ // For list after insert
          ...mockTasks,
          {
      ...
      const writtenContent = JSON.parse(vi.mocked(mockFilesystem.writeFile).mock.calls[0][1]);
  ...
  describe('delete', () => {
    it('should delete a task and return the updated list', async () => {
      ...
      if (E.isRight(result)) {
        expect(result.right).toHaveLength(mockTasks.length - 1);
        expect(result.right.find(task => task.id === taskIdToDelete)).toBeUndefined();
      }
      expect(mockFilesystem.writeFile).toHaveBeenCalledTimes(1);
      const writtenContent = JSON.parse(vi.mocked(mockFilesystem.writeFile).mock.calls[0][1]);
      expect(writtenContent).toHaveLength(mockTasks.length - 1);
      expect(writtenContent.find((task: TaskEncoded) => task.id === taskIdToDelete)).toBeUndefined();
    });
  ...
  describe('update', () => {
    it('should return the current list if task to update is not found', async () => {
      ...
      // Act
      const result = await runTaskTracker(TaskTracker.update(nonExistentId, updates));
