...
import { Priority, Status, TaskId, Description } from '@/schema.simple'
...
const byPriority = Ord.contramap<number, Task>(task => priorityLookup(task.priority))(N.Ord)
const byStatus = Ord.contramap<number, Task>(task => statusLookup(task.status))(N.Ord)
const byCreationDate = Ord.contramap<Date, Task>(task => task.createdAt)(D.Ord)
const byUpdateDate = Ord.contramap<O.Option<Date>, Task>(task => task.updatedAt)(O.getOrd(D.Ord))
const byDescription = Ord.contramap<string, Task>(task => task.description)(S.Ord)
