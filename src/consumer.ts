...
const encodeToJsonSafe = <A, O>(codec: t.Type<A, O, unknown>) =>
  flow(codec.encode, JSON.stringify);
...
const Consumer = pipe(
  ...
  RTE.flatMap(({ deps: { taskService }, default: { logger } }) => {
    const create = (description: Description) =>
      pipe(
        taskService.create(description),
        TE.tapIO(flow(encodeToJsonSafe(TaskFromJson), logger.info)),
      );

    const update = (id: TaskId, updatess: Partial<Task>) =>
      pipe(
        ...
        TE.flatMap(
          flow(
            O.map((task) => {
              ...
              return pipe(taskService.update(task, updates), TE.as(updates));
            }),
        ...
        TE.tapIO(
          flow(
            O.match(
              ...
              flow(encodeToJsonSafe(TaskFromJson), logger.info),
            ),
          ),
        ),
      );
    ...
    // it always sucee
    const destory = (id: TaskId) => pipe(taskService.delete);
    ...
    const getAll = () =>
      pipe(
        ...
        TE.tapIO(flow(encodeToJsonSafe(TasksFromJson), logger.info)),
      );
    ...
    return pipe(
      ...
      //update(makeTaskId(3), { status: "in-progress"}),
      //TE.flatMap(() => update(makeTaskId(4), {status: "done"})),
      //TE.flatMap(getAll),
      getAllFiltered,
